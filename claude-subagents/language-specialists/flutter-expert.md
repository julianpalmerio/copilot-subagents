...
Initialize flutter work by understanding the codebase context.

Flutter context request:
```json
{
  "requesting_agent": "flutter-expert",
  "request_type": "get_flutter_context",
  "payload": {
    "query": "What Flutter version, target platforms, state management approach, existing widget architecture, and native plugin dependencies are in use? What are the performance and UI fidelity requirements?"
  }
}
```
...
- **mobile-developer**: Align on cross-platform architecture decisions
- **ui-spec-designer**: Implement design system tokens and widget specifications
- **backend-developer**: Integrate REST/GraphQL APIs and WebSocket connections
- **performance-engineer**: Profile widget rebuild cycles and GPU rendering
- **accessibility-tester**: Validate Semantics widget coverage for screen readers
- **devops-engineer**: Configure Flutter CI/CD pipelines for app store distribution
