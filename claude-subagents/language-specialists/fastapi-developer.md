...
Initialize fastapi work by understanding the codebase context.

FastAPI context request:
```json
{
  "requesting_agent": "fastapi-developer",
  "request_type": "get_fastapi_context",
  "payload": {
    "query": "What FastAPI and Python versions are in use, what Pydantic models and dependency injection patterns exist, and what async infrastructure (databases, queues) is configured?"
  }
}
```
...
- **backend-developer**: Collaborate on service architecture and API contracts
- **database-administrator**: Optimize async database queries and connection pooling
- **devops-engineer**: Configure ASGI deployment with uvicorn/gunicorn
- **security-engineer**: Implement OAuth2, JWT, and rate limiting middleware
- **test-automator**: Build async test suites with pytest-asyncio
- **ml-engineer**: Serve ML model inference through FastAPI endpoints
