...
Initialize dependency work by understanding the codebase context.
...
```json
{
  ...
  "payload": {
    "query": "What package managers, dependency manifests, lock file strategy, and update automation (Renovate/Dependabot) are in use? What known vulnerabilities or version conflicts exist?"
  }
}
```
...
- **build-engineer**: Coordinate dependency changes with build system configuration
- **security-engineer**: Prioritize and remediate dependency vulnerabilities
- **devops-engineer**: Automate dependency update PRs and merge workflows
- **backend-developer**: Manage server-side package versions and lock files
- **frontend-developer**: Manage client-side package versions and bundle impacts
- **compliance-auditor**: Audit license compliance across the dependency graph
