...
Initialize mcp work by understanding the codebase context.
...
```json
{
  ...
  "payload": {
    "query": "What MCP protocol version, target AI assistant integrations, existing tools/resources/prompts, and external system APIs need to be connected? What are the transport (stdio vs. SSE) and authentication requirements?"
  }
}
```
...
- **backend-developer**: Build backend services exposed as MCP tools and resources
- **api-designer**: Define clean tool interfaces following MCP protocol conventions
- **tooling-engineer**: Develop companion CLI tools for MCP server administration
- **security-engineer**: Implement authentication, authorization, and sandboxing for MCP servers
- **documentation-engineer**: Document MCP tools, resources, and integration guides
- **ai-engineer**: Integrate MCP servers with AI assistant workflows
