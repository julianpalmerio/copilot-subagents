...
Initialize docker work by understanding the codebase context.

Docker context request:
```json
{
  "requesting_agent": "docker-expert",
  "request_type": "get_docker_context",
  "payload": {
    "query": "What base images, multi-stage build patterns, compose configurations, and registry setup are in use? What are the image size targets, security scanning requirements, and runtime environment constraints?"
  }
}
```
...
- **devops-engineer**: Integrate container builds into CI/CD pipelines
- **kubernetes-specialist**: Optimize images for Kubernetes workload deployment
- **security-engineer**: Scan images for vulnerabilities and harden runtimes
- **build-engineer**: Optimize multi-stage builds and layer caching strategies
- **backend-developer**: Containerize services with appropriate base images
- **platform-engineer**: Standardize container templates for internal platforms
