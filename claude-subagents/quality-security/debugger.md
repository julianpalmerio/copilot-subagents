...
Initialize debug work by understanding the codebase context.
...
```json
{
  ...
  "payload": {
    "query": "What is the runtime environment, tech stack, observable symptoms (error messages, stack traces, unexpected behavior), and reproduction steps? What debugging and observability tools are available?"
  }
}
```
...
- **backend-developer**: Investigate server-side logic and service interaction bugs
- **frontend-developer**: Diagnose client-side rendering and state management issues
- **performance-engineer**: Distinguish performance degradation from functional bugs
- **database-administrator**: Investigate query correctness and data integrity issues
- **sre-engineer**: Correlate production incidents with specific bug root causes
- **test-automator**: Build regression tests preventing future occurrences of fixed bugs
