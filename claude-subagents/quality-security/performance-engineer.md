...
Initialize performance work by understanding the codebase context.
...
```json
{
  ...
  "payload": {
    "query": "What are the current baseline performance metrics (latency, throughput, CPU/memory), what SLAs are targets, what profiling and APM tools are available, and where are the known bottlenecks?"
  }
}
```
...
- **backend-developer**: Optimize service logic, caching, and database query patterns
- **frontend-developer**: Improve Core Web Vitals, bundle sizes, and rendering performance
- **database-administrator**: Optimize query execution plans and indexing strategies
- **sre-engineer**: Correlate performance metrics with SLO compliance
- **cloud-architect**: Right-size compute resources and configure CDN/caching layers
- **build-engineer**: Reduce build and test execution times
