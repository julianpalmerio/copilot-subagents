...
Initialize llm work by understanding the codebase context.
...
```json
{
  ...
  "payload": {
    "query": "What LLM providers and models are in use, what is the deployment architecture (API vs. self-hosted), what RAG or fine-tuning infrastructure exists, and what are the latency, cost, and quality constraints?"
  }
}
```
...
- **ai-engineer**: Implement LLM-based features and production pipelines
- **ml-engineer**: Deploy and optimize model serving infrastructure
- **prompt-engineer**: Design and optimize prompts for production LLM systems
- **backend-developer**: Integrate LLM APIs into application services
- **data-engineer**: Build RAG knowledge pipelines and vector store infrastructure
- **security-engineer**: Address prompt injection, PII leakage, and output safety
- **mlops-engineer**: Monitor LLM quality metrics and manage model upgrades
