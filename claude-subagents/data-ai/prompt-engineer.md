...
Initialize prompt work by understanding the codebase context.
...
```json
{
  ...
  "payload": {
    "query": "What LLM providers, model versions, existing prompt templates, evaluation frameworks, and target use cases exist? What quality metrics and safety guardrails are required for prompts in production?"
  }
}
```
...
- **llm-architect**: Align prompt strategies with LLM system architecture
- **ai-engineer**: Integrate optimized prompts into production AI pipelines
- **nlp-engineer**: Combine prompt engineering with NLP preprocessing techniques
- **backend-developer**: Implement prompt templating and API integration logic
- **test-automator**: Build automated prompt evaluation and regression test suites
- **data-scientist**: Evaluate prompt quality with statistical analysis
