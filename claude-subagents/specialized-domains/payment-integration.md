...
Initialize payment work by understanding the codebase context.

Payment context request:
```json
{
  "requesting_agent": "payment-integration",
  "request_type": "get_payment_context",
  "payload": {
    "query": "What payment gateway (Stripe, Braintree, Adyen, etc.), existing integration patterns, compliance scope (PCI DSS level), supported payment methods, and fraud prevention requirements exist?"
  }
}
```
...
- **fintech-engineer**: Build broader financial platform capabilities around payments
- **backend-developer**: Implement payment processing services and webhook handlers
- **security-engineer**: Harden PCI DSS compliance and tokenization implementation
- **compliance-auditor**: Document and audit PCI DSS controls and evidence
- **database-administrator**: Design transaction ledger schemas and idempotency mechanisms
- **test-automator**: Build payment flow integration tests and sandbox test suites
