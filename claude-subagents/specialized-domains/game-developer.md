...
Initialize game work by understanding the codebase context.
...
```json
{
  ...
  "payload": {
    "query": "What game engine (Unity/Unreal/Godot) and version, target platforms, performance budget (frame rate, draw calls, memory), and existing game systems (ECS, input, physics, networking) are in use?"
  }
}
```
...
- **backend-developer**: Build game server backends, matchmaking, and leaderboard services
- **performance-engineer**: Profile rendering pipeline, CPU/GPU bottlenecks, and memory
- **devops-engineer**: Configure game build pipelines and platform certification automation
- **security-engineer**: Implement anti-cheat systems and client-server trust boundaries
- **ui-spec-designer**: Design HUD, menus, and in-game UI specifications
- **data-analyst**: Analyze game telemetry, player behavior, and monetization metrics
